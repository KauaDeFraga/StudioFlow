...
export function InstructorCard({ instructor, onEdit, onDelete }: InstructorCardProps) {
  ...
  return (
    <Card data-testid={`instructor-card-${instructor.id}`} className="border-l-4 border-l-primary">
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-3">
        <div className="flex items-center gap-3">
          <Avatar className="w-12 h-12 bg-primary">
            <AvatarFallback className="bg-transparent text-primary-foreground font-bold">{getInitials(instructor.name)}</AvatarFallback>
          </Avatar>
          <div>
